...
which changes along the freezeout surface as
    ...
    \end{equation}
with $\dt^\mu s=(\partial x^\mu)/(\partial\alpha)\dt\alpha\equiv(\tau^\prime,r^\prime)\dt\alpha$ the displacement vector on the freezeout surface. To prevent confusion with the signature, note
\begin{equation}
    \dt\varphi=\dt\tau\partial_\tau\varphi+\dt r\partial_r\varphi=\dt\alpha(\tau^\prime u_\tau+r^\prime u_r)\chi=\dt\alpha(-\tau^\prime u^\tau+r^\prime u_r)\equiv\chi u_\mu\dt^\mu s
\end{equation}
For this prescription, the solution $\varphi$ on the freezout surface thus needs to fulfill the ODE
\begin{equation}
...
The 4-vector $u^\mu$ is timelike, ${u_\mu u^\mu=-1}$ and future oriented, $u^\tau>0$. Long after the collision, when the fireball expands and density decreases, the amplitude of the condensate can be expected to decrease in direction of the fluid flow, ${\dt(\varphi)^2\sim\varphi\dt\varphi<0}$ where
\begin{equation}
    \dt\varphi=(u^\tau\partial_\tau\varphi+u^r\partial_r\varphi)\dt\tau_E=u^\mu\partial_\mu\varphi\dt\tau_E
\end{equation}
with the eigen time interval $\dt\tau_E>0$. Inserting ${\partial_\mu\varphi=\chi u_\mu}$ gives
\begin{equation}
    \dt\varphi=\chi u_\mu u^\mu\dt\tau_E=-\chi\dt\tau_E\,.
\end{equation}
In this case, $\varphi\chi>0$ is the appropiate choice for a decreasing density of the condensate in the frame of the expanding fireball.
...
\end{equation}
Further imposing $(\partial_\mu\pi^a)\sim u_\mu$ leads to 
\begin{equation}
...
Considering the fact that we have already proposed ways to initialize the real valued fields $\pi^1$, $\pi^2$, we could also just simply construct $\pi^\pm=(1/\sqrt{2})(\pi^1\mp\imagu\pi^2)$ from these fields. Given $\partial_\mu\pi^{1,2}$, the density of the charged pion fields changes as
\begin{equation}
