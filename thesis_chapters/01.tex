...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
only subject to the condition $\omega_{\vec{p}}=\sqrt{m^2+p^2}$. Reality of $\phi(t,x)$ implies $a_{\vec{p}}=b_{\vec{p}}$.
If one uses
...
\end{equation}
lifting fields to operators, $\phi\to\hat{\phi}$ and $\pi\to\hat{\pi}$ (though the $\hat{\cdot}$ will be omitted) as well as $\cdot^*\to\cdot^\dagger$. Using $a_{\vec{p}}=b_{\vec{p}}$ together with the normalization $\mathcal{N}_{\vec{p}}^2\omega_{\vec{p}}=\frac{1}{2}$, the fundamental commutator $\commut{\phi(t,x),\pi(t,y)}=\imagu\delta^{(d)}(x-y)$ then implies
