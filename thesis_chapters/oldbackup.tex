...
\begin{equation}
    \left\{\begin{split}
        p^t&=m_\perp\cosh\eta_p\\
        p^z&=m_\perp\sinh\eta_p\\
        p^x&=p_\perp\cos\varphi_p\\
        p^y&=p_\perp\sin\varphi_p
    \end{split}\right.
\end{equation}
to rewrite the scalar product as
\begin{equation}
    p_\mu x^\mu\equiv-\tau(p^t\cosh\eta-p^z\sinh\eta)+r(p^x\cos\varphi+p^y\sin\varphi)=-\tau m_\perp\cosh(\eta-\eta_p)+r p_\perp\cos(\varphi-\varphi_p)
\end{equation}
...
The red graphs represent field configurations that are approximately constant on the $\tau\approx\const$ subsection of the freezeout surface. They seem to generate a distinct "break" in the spectrum, where the average slope (in the log scale) changes abruptly.
